...
def main():
    ...
    if gan_type == 'pggan':
        ...
        if generator.c_dim == 0:
            c = torch.zeros(codes.size(0), 0).cuda() #No conditioning
        else:
            # Create a zero-conditioning vector (for unconditional models)
            c = torch.zeros(codes.size(0), generator.c_dim).cuda()

        # Map latent codes to the intermediate latent space
    mapped_codes = generator.mapping(codes, c)
    print(f"Mapped codes type: {type(mapped_codes)}, value: {mapped_codes}")
    codes = mapped_codes  # Update based on actual structure of mapped_codes

    codes = generator.truncation(codes, args.trunc_psi, args.trunc_layers)
    ...
    print(f"num_sem: {num_sem}, values.shape: {values.shape}") #debug statement
    num_sem = min(num_sem, values.shape[0])  # Ensure we don't exceed available values
